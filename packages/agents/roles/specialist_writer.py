...
class SpecialistWriterInput(BaseModel):
    ...
    domain: str = Field(
        default="general", description="Domain context: finance, legal, healthcare, etc."
    )
...
class SpecialistWriterAgent(BaseAgent):
    ...
    description = (
        "Domain-aware technical writer that produces polished, audience-appropriate documents."
    )
    ...
    async def _execute(
        self, input: SpecialistWriterInput, context: CallContext
    ) -> SpecialistOutput:
